...
@Injectable()
export class EmpresavinculaService {
  ...
  public inserir(fornecedorClienteDadosAcesso: any): Observable<GenericResult<any>> {
    return this.http.post(this.commonService.getBaseUrl() + '/fornecedorclientedadosacesso/Incluir', JSON.stringify(fornecedorClienteDadosAcesso), {
  ...
  public delete(id: number): Observable<GenericSimpleResult> {
    return this.http.delete(this.commonService.getBaseUrl() + '/fornecedorcliente/' + id, { headers: this.headers })
      .map(res => res.json());
  }
